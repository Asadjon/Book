...
class FavouriteDatabaseController(context: Context, mFileName: String) : DatabaseCopyFromAssets(context, mFileName) {
    ...
    var chapterList: MutableList<Data>
        get() {
            return StaticClass.getJsonListAt(jsonObject?.let { getValue(it, "Contents") } as JSONArray) { chapterJson: JSONObject ->
                val chapter =
                    Chapter(getValue(chapterJson, "Chapter") as Int,
                        getValue(chapterJson, "Name") as String?,
                        getValue(chapterJson, "Value") as String?,
                        Pages(
                            StaticClass.getJsonListAt(
                                getValue(
                                    chapterJson,
                                    "Pages"
                                ) as JSONArray
                            ) { target: Int? -> target }.toTypedArray()
                        )
                    )

                if (chapterJson.has("Themes"))
                    Chapter(chapter)
                        .set(StaticClass.getJsonListAt(getValue(chapterJson, "Themes") as JSONArray) { themeJson: JSONObject ->
                            Theme(
                                getValue(
                                    themeJson,
                                    "Id"
                                ...
                                Pages(
                                    StaticClass.getJsonListAt<Int, Int>(
                                        getValue(
                                            themeJson,
                                            "Pages"
                                        ) as JSONArray
